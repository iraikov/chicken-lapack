...
(define-syntax atlas-wrapx
  (lambda (x r c)
    (let* ((fn     (cadr x))
           (ret    (caddr x))
           (errs   (cadddr x)))
      `(begin
         (atlas-wrap ,(cons (string->symbol (conc "s" (symbol->string (car fn)))) (cdr fn))
                     ,ret ,errs #f #f)
         (atlas-wrap ,(cons (string->symbol (conc "d" (symbol->string (car fn)))) (cdr fn))
                     ,ret ,errs #f #f)
         (atlas-wrap ,(cons (string->symbol (conc "c" (symbol->string (car fn)))) (cdr fn))
                     ,ret ,errs #f #f)
         (atlas-wrap ,(cons (string->symbol (conc "z" (symbol->string (car fn)))) (cdr fn))
                     ,ret ,errs #f #f)
  ...
  (atlas-wrapx (gesv order n nrhs a lda opiv b ldb)
    (a b opiv)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) "upper triangular matrix is singular")))


  (atlas-wrapx (posv order uplo n nrhs a lda b ldb)
    (a b)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) (conc "leading minor of order " i
           " of A is not positive definite"))))

  (atlas-wrapx (getrf order m n a lda opiv)
    (a opiv)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) "factor U is singular")))

  (atlas-wrapx (getrs order trans n nrhs a lda ipiv b ldb)
    (b)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) "unknown error")))

  (atlas-wrapx (getri order n a lda ipiv)
    (a)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) "factor U is singular")))

  (atlas-wrapx (potrf order uplo n a lda)
    (a)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) (conc "leading minor of order " i " is not positive definite"))))

  (atlas-wrapx (potrs order uplo n nrhs a lda b ldb)
    (b)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) "unknown error")))

  (atlas-wrapx (potri order uplo n  a lda)
    (a)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) (conc "element " "(" i "," i")" " of factor U or L is zero"))))

  (atlas-wrapx (trtri order uplo diag n a lda)
    (a)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) "the triangular matrix is singular")))

  (atlas-wrapx (lauum order uplo n a lda)
    (a)
    ((lambda (i) (conc i "-th argument had an illegal value"))
     (lambda (i) "unknown error")))
